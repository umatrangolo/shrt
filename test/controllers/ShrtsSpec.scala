...
class ShrtsSpec extends PlaySpecification {
  ...
  "The Shrts controller" should {
    ...
    // }
    "return a 301 to the original URL for a given token" in new WithServer(
      app = FakeApplication(additionalConfiguration = Helpers.inMemoryDatabase(name = "shrt")),
      port = 19000
    ) {
      println(app)
      val Some(result) = route(FakeRequest(GET, "/shrts/googl"))
      status(result) must equalTo(OK)
    }
    ...
    // }
    // "return a 200 on a delete on an existing token" in new WithServer(port = 19000) {
    //   val Some(result) = route(FakeRequest(DELETE, "/shrts/googl"))
    //   status(result) must equalTo(OK)
    // }
    ...
    //   status(result) must equalTo(404)
    // }
    // "return a 200 for a list of all the known tokens" in new WithServer(port = 19000) {
